...
const parseChildren = (context, tags) => {
    const nodes = [];
...
function parseElement(context, tags) {
...
function startsWithEndTagOpen(source, tag) {
    return (source.startsWith(endTag) &&
...
const transformElement = (node, context) => {
    const nodeVal = node;
    if (nodeVal.type === 2 /* NodeTypes.ELEMENT */) {
        return () => {
            // tag
            ...
            nodeVal.codegenNode = createVNodeCall(context, vnodeTag, vnodeProps, vnodeChildren);
        };
    }
};
...
function transformText(nodeArg) {
    const node = nodeArg;
